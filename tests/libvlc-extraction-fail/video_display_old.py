...
class VideoDisplay():
    ...
    def __init__(self, videopath, timestamp, deque_size, video_scale=1.0):
        self.get_frame_thread = Thread(target=self.get_timestamp, args=([timestamp]))
    ...
    def video_pre_proc(self, videopath, save_csv_path):
        global pplayer
        pplayer = vlc.MediaPlayer(videopath)
        ...
        vlc.libvlc_video_set_callbacks(pplayer, _lockcb, None, _display, None)
        pplayer.video_set_format("RV32", VIDEOWIDTH, VIDEOHEIGHT, VIDEOWIDTH * 4)

        pplayer.play()
        vlen = pplayer.get_length()
        while True:
            if pplayer.get_time() >= vlen:
                break
        ...
        #         timestamp = frame_player.get_time()
        #         print(timestamp)
        #         print(vectors)
    ...
    # Pre process the video and save the angular velocity of the instructor in a separate file
    # the craeted file saves the: [ frame number, right_elbow, right_shoulder, left_elbow, left_shoulder ]
    # TODO rewrite all this process using VLC library instead of OpenCV. Because OpenCV video timming is shit !!
    # def video_pre_proc(self, videopath, save_csv_path):
    ...
    # Get the video timestamp (For Training use only) 
